    ...
    <Layout>
        <template #header>
            <div class="flex flex-row justify-between items-center w-100">
                ...
                <h1 class="heading">{{ tasksListsStore.translate('pages.newTask.newTask') }}</h1>
                <div></div>
            </div>
        </template>
        ...
        <template #main>
            <v-form validate-on="submit" @submit.prevent="addTask" class="flex flex-col items-center">
                ...
                <v-text-field class="w-100" type="datetime-local" v-model="taskRemindMeAtDateTime" clearable
                 ...
                 <v-btn class="w-full sm:w-auto" color="blue-grey-darken-3" type="submit" size="large" :disabled="!taskTitle || taskTitle.trim() === ''">{{
